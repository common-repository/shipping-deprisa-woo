...
class Shipping_Deprisa_WC_plugin
{
    ...
    public function run_deprisa_wc(): void
    {
        try{
    ...
    public function plugin_action_links($links): array
    {
        ...
        $plugin_links[] = '<a target="_blank" href="https://shop.saulmoralespa.com/shipping-deprisa-woo/">' . 'Documentación' . '</a>';
        return array_merge( $plugin_links, $links );
    }

    public function shipping_deprisa_wc_add_method( $methods ): array
    {
    ...
    public function custom_woocommerce_fields($fields)
    {
        ...
        if (!str_contains($chosen_shipping, 'shipping_deprisa_wc'))
            return $fields;
    ...
    public function check_post_code()
    {
        ...
        if (strlen($post_code) !== 6 || !Shipping_Deprisa_WC::is_acepted_post_code($state, $post_code))
    ...
    public function print_label($columns) : array
    {
    ...
    public function enqueue_scripts_admin($hook): void
    {
        if ($hook === 'edit.php'){
            ...
            wp_enqueue_script( 'config_shipping_deprisa_wc_sd', $this->plugin_url . 'assets/js/shipping-deprisa-wc.js', array( 'jquery' ), $this->version, true );
        }
    }
